module fortio_deflate_checksums
    !! CRC32 and Adler-32 checksums used by Deflate and ZIP.
...
contains
    ...
    function calculate_adler32(data, data_len) result(adler32)
        ...
        i = 1
        do while (i <= data_len)
            block_end = min(i + 5551, data_len)
            do while (i <= block_end)
                s1 = s1 + iand(int(data(i), int64), 255_int64)
                s2 = s2 + s1
                i = i + 1
            end do
            s1 = modulo(s1, 65521_int64)
